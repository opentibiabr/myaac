...
if ('post' == strtolower($method)) {
  ...
  if ($type === 'transaction') {
    try {
      ...
      $transactionDB = $db
        ->query("SELECT * FROM `pagseguro_transactions` WHERE `transaction_code` = {$db->quote($transaction_code)} AND `account_id` = {$account_id}")
        ->fetch();
      ...
      if (!($id = $transactionDB['id'] ?? null)) {
        ...
        $values = "{$db->quote($transaction_code)}, {$account_id}, {$db->quote($payment_method)}, {$db->quote($payment_status)}, {$db->quote(
          $donateSelected['id']
        )}, {$coins_amount}, {$bought}, {$is_doubled}, {$db->quote($request)}, {$db->quote($createdAt)}";
        $db->exec(
      ...
      if (
        ...
        if ($account_id) {
          ...
          $db->exec(
            "UPDATE `pagseguro_transactions` SET `delivered` = 1, `request` = {$db->quote($request)}, `updated_at` = {$db->quote(
              $updateAt
            )} WHERE `id` = {$id}"
          );
        ...
        $db->exec(
          "UPDATE `pagseguro_transactions` SET `request` = {$db->quote($request)}, `updated_at` = {$db->quote($updateAt)} WHERE `id` = {$id}"
        );
