...
if ('post' == strtolower($method)) {
    ...
    if ($type === 'transaction') {
        try {
            ...
            $transactionDB    = $db->query("SELECT * FROM `pagseguro_transactions` WHERE `transaction_code` = {$db->quote($transaction_code)} AND `account_id` = {$account_id}")->fetch();
            if (!$donateSelected = $config['pagSeguro']['donates'][$transaction->getItems()[0]->getId()] ?? null) {
                return false;
            }
            ...
            if (!$id = $transactionDB['id'] ?? null) {
                ...
                $values       = "{$db->quote($transaction_code)}, {$account_id}, {$db->quote($payment_method)}, {$db->quote($payment_status)}, {$db->quote($donateSelected['id'])}, {$coins_amount}, {$bought}, {$is_doubled}, {$db->quote($request)}, {$db->quote($createdAt)}";
                $db->exec("INSERT INTO `pagseguro_transactions` (`transaction_code`, `account_id`, `payment_method`, `payment_status`, `code`, `coins_amount`, `bought`, `in_double`, `request`, `created_at`) VALUES ({$values})");
                $transactionDB = $db->query("SELECT * FROM `pagseguro_transactions` WHERE `id` = {$db->lastInsertId()}")->fetch();
                $id = $transactionDB['id'];
            }
            ...
            if ($transactionDB['delivered'] == "0" && (($payment_method == 'CREDIT_CARD' && $payment_status == 'PAID') || ($payment_method == 'PIX' && $payment_status == 'AVAILABLE'))) {
                ...
                if ($account_id) {
                    ...
                    $db->exec("UPDATE `pagseguro_transactions` SET `delivered` = 1, `request` = {$db->quote($request)}, `updated_at` = {$db->quote($updateAt)} WHERE `id` = {$id}");
                    ...
                    $values = "{$account_id}, 1, {$coins_amount}, {$db->quote('Donate')}, {$db->quote($updateAt)}, 3";
                    ...
                    $values2   = "{$account_id}, 0, {$db->quote('Donate')}, 3, {$coins_amount}, {$timestamp}, 0, 0";
                    $db->exec("INSERT INTO `store_history` (`account_id`, `mode`, `description`, `coin_type`, `coin_amount`, `time`, `timestamp`, `coins`) VALUES ({$values2})");
                }
            } else {
                $db->exec("UPDATE `pagseguro_transactions` SET `request` = {$db->quote($request)}, `updated_at` = {$db->quote($updateAt)} WHERE `id` = {$id}");
