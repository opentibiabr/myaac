...
if (!$error) {
  ...
  if (isset($database_error)) {
    // we failed connect to the database
...
if (!$db->hasColumn('accounts', 'created')) {
  if (
    query(
      'ALTER TABLE `accounts` ADD `created` INT(11) NOT NULL DEFAULT 0 AFTER `' .
        ($db->hasColumn('accounts', 'group_id') ? 'group_id' : 'email') .
        '`;'
    )
  ) {
...
} elseif (!$db->hasColumn('accounts', 'web_lastlogin')) {
...
} elseif (!$db->hasColumn('accounts', 'email_next')) {
...
} elseif (!$db->hasColumn('guilds', 'logo_name')) {
...
} elseif (!$db->hasColumn('players', 'hidden')) {
