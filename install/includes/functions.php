...
function next_buttons($previous = true, $next = true)
{
  ...
  if ($previous) {
    $ret .=
      '<input type="button" class="btn btn-primary mx-2" onclick="document.getElementById(\'step\').value=\'' .
      $steps[$i - 1] .
      '\'; this.form.submit();" value="&laquo; ' .
      $locale['previous'] .
      '" />';
  }
  if ($next) {
    $ret .=
      '<input type="button" class="btn btn-success mx-2" onclick="document.getElementById(\'step\').value=\'' .
      $steps[$i + 1] .
