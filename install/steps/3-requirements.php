...
// configuration
$extensions_required = ['pdo', 'pdo_mysql', 'xml', 'zip'];
/*
 *
 * @param string $name
 * @param boolean $ok
...
foreach ($extensions_required as $ext) {
  ...
  version_check(
    str_replace('$EXTENSION$', strtoupper($ext), $locale['step_requirements_extension']),
    $loaded,
    $loaded ? $locale['loaded'] : $locale['not_loaded']
  );
}
