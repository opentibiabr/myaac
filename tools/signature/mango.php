...
/** Default values **/
list($i, $eachRow, $percent) = [0.5, 14, ['size' => 7]];
...
$MadGD->addText($player->getName(), $player->isOnline() ? ['color' => '5df82d'] : [])->setPosition();
$i++;
/** SEX **/
...
$MadGD->addText($player_sex)->setPosition();
$i++;
/** PROFESSION **/
$vocation = 'Unknown';
...
$MadGD->addText($vocation)->setPosition();
$i++;
/** LEVEL **/
$MadGD->addText('Level:', $MadGD->textBold)->setPosition(10, $i * $eachRow);
$MadGD->addText($player->getLevel())->setPosition();
$i++;
/** WORLD **/
if ($config['multiworld']) {
  $MadGD->addText('World:', $MadGD->textBold)->setPosition(10, $i * $eachRow);
  $MadGD->addText($config['worlds'][$player->getWorldId()])->setPosition();
  $i++;
}

/** RESIDENCE **/
$MadGD->addText('Residence:', $MadGD->textBold)->setPosition(10, $i * $eachRow);
$MadGD->addText($config['towns'][$player->getTownId()])->setPosition();
$i++;
...
$house = $db
  ->query('SELECT `houses`.`name`, `houses`.`' . $town . '` as town FROM `houses` WHERE `houses`.`owner` = ' . $player->getId() . ';')
  ->fetchAll();
if (count($house) != 0) {
  $MadGD->addText('House:', $MadGD->textBold)->setPosition(10, $i * $eachRow);
  $MadGD->addText($house[0]['name'] . ' (' . $towns_list[$player->getWorldId()][$house[0]['town']] . ')')->setPosition();
  $i++;
}
/** GUILD **/
$rank = $player->getRank();
if ($rank->isLoaded()) {
  ...
  $MadGD->addText($rank->getName() . ' of the ' . $player->getRank()->getGuild()->getName())->setPosition();
  $i++;
}
/** LAST LOGIN **/
$MadGD->addText('Last login:', $MadGD->textBold)->setPosition(10, $i * $eachRow);
$MadGD->addText($player->getLastLogin() == 0 ? 'Never logged in' : date('M d Y, H:i:s T', $player->getLastLogin()))->setPosition();
$i++;
/** ACCOUNT STATUS **/
$MadGD->addText('Account Status:', $MadGD->textBold)->setPosition(10, $i * $eachRow);
$MadGD->addText($player->getAccount()->getPremDays() > 0 ? 'Premium Account' : 'Free Account')->setPosition();
$i++;
...
$slots = [
  2 => [$MadGD->equipment['x']['amulet'], $MadGD->equipment['y']['amulet']],
  1 => [$MadGD->equipment['x']['helmet'], $MadGD->equipment['y']['helmet']],
  3 => [$MadGD->equipment['x']['backpack'], $MadGD->equipment['y']['backpack']],
  6 => [$MadGD->equipment['x']['lefthand'], $MadGD->equipment['y']['lefthand']],
  4 => [$MadGD->equipment['x']['armor'], $MadGD->equipment['y']['armor']],
  5 => [$MadGD->equipment['x']['righthand'], $MadGD->equipment['y']['righthand']],
  9 => [$MadGD->equipment['x']['ring'], $MadGD->equipment['y']['ring']],
  7 => [$MadGD->equipment['x']['legs'], $MadGD->equipment['y']['legs']],
  10 => [$MadGD->equipment['x']['ammunition'], $MadGD->equipment['y']['ammunition']],
  8 => [$MadGD->equipment['x']['boots'], $MadGD->equipment['y']['boots']],
];
foreach ($slots as $pid => $position) {
  $item = $db
    ->query(
      'SELECT `itemtype`, `attributes` FROM `player_items` WHERE `player_items`.`player_id` = ' .
        $player->getId() .
        ' AND `player_items`.`pid` = ' .
        $pid .
        ';'
    )
    ->fetch();
