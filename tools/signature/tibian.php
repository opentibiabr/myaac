...
if ($db->hasColumn('players', 'skill_fist')) {
  ...
  $skills_db = $db
    ->query(
      'SELECT `skill_fist`, `skill_club`, `skill_sword`, `skill_axe`, `skill_dist`, `skill_shielding`, `skill_fishing` FROM `players` WHERE `id` = ' .
        $player->getId()
    )
    ->fetch();
  ...
  $skills = $db->query(
    'SELECT ' .
      $db->fieldName('skillid') .
      ', ' .
      $db->fieldName('value') .
      ' FROM ' .
      $db->tableName('player_skills') .
      ' WHERE ' .
      $db->fieldName('player_id') .
      ' = ' .
      $player->getId() .
      ' LIMIT 7'
  );
