...
class MadGD
{
  public $textConfig = ['font' => 2, 'size' => 9, 'color' => 'FFFFFF', 'shadow' => true];
  public $textBold = ['font' => 3, 'size' => 9, 'color' => 'FFFFFF', 'shadow' => true];
  ...
  {
    $this->textConfig = ['font' => $regular, 'size' => $size, 'color' => $color, 'shadow' => $shadow];
    $this->textBold = ['font' => $bold, 'size' => $size, 'color' => $color, 'shadow' => $shadow];
  }
  ...
  public function addText($text, $style = [])
  {
    ...
    return new MadGDText($this, [$text, $style]);
  }
  ...
  public function HexRGB($hex, $index = false)
  {
    if (!is_array($hex)) {
      ...
      if (strlen($hex) == 6) {
        $hex = [hexdec(substr($hex, 0, 2)), hexdec(substr($hex, 2, 2)), hexdec(substr($hex, 4, 2))];
      }
    }
    return in_array($index, [0, 1, 2]) ? $hex[$index] : $hex;
  }
    ...
    if (is_int($this->style['font'])) {
      if ($this->style['shadow']) {
        imagestring(
          ...
          $this->parent->y + 1,
          $this->text,
          imagecolorallocate($this->parent->instance, 0, 0, 0)
        );
      }
      imagestring(
        $this->parent->instance,
        ...
        imagecolorallocate(
          $this->parent->instance,
          $this->parent->HexRGB($this->style['color'], 0),
          $this->parent->HexRGB($this->style['color'], 1),
          $this->parent->HexRGB($this->style['color'], 2)
        )
      );
    } else {
      if ($this->style['shadow']) {
...
class MadGDIcon
{
  ...
  public function setPosition($x = MADGD_STACK, $y = 5)
  {
    ...
    return imagecopyresampled(
      ...
      0,
      0,
      $this->width != null ? $this->width : $imageWidth,
      $this->height != null ? $this->height : $imageHeight,
      $imageWidth,
      $imageHeight
    );
  }
