...
if (!extension_loaded('curl')) {
  error(
    'cURL php extension is not loaded, please install it with following command (on linux):' .
      '<br/>' .
      'sudo apt-get install php5-curl' .
      '<br/>' .
      'sudo service apache2 restart' .
      '<br/><br/>' .
      'for XAMPP (Windows) you need to uncomment (Remove selicolon - ;) this line in your php.ini:' .
      '<br/>' .
...
if ($is_localhost && !$config['enablePagseguroLocal']) {
  warning(
    'PagSeguro is not supported on localhost (' .
      BASE_URL .
      "). Please change your domain to public one and visit this site again later.<br/>
...
if (empty($action)) {
  if (!$logged) {
    ...
    echo 'To buy coins you need to be logged. ' .
      generateLink(getLink('?subtopic=accountmanagement') . '&redirect=' . urlencode(BASE_URL . '?subtopic=boxes'), 'Login') .
      ' first to buy a surprise box.';
