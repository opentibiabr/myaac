...
if ($action == 'step3') {
          ...
          if (Validator::password($new_pass)) {
            if (Validator::email($new_email)) {
              ...
              echo 'Your account name, new password and new e-mail.<BR>
							...
							<TR><TD BGCOLOR="' .
                $config['vdarkborder'] .
                '" class="white"><B>Your account name, new password and new e-mail</B></TD></TR>
							<TR><TD BGCOLOR="' .
                $config['darkborder'] .
                '">
							Account name:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>' .
              ...
              if ($account->getCustomField('email_next') < time()) {
                $mailBody =
                  '
								...
								<p>Changed password and e-mail to your account in Lost Account Interface on server <a href="' .
                  BASE_URL .
                  '"><b>' .
              ...
              echo '<INPUT TYPE=hidden NAME="account_login" VALUE="' .
                $account->getId() .
                '">
							<INPUT TYPE=hidden NAME="password_login" VALUE="' .
                $new_pass .
                '">
							</TD></TR></TABLE><BR>
  ...
  echo '<BR /><TABLE CELLSPACING=0 CELLPADDING=0 BORDER=0 WIDTH=100%><TR><TD><div style="text-align:center">
				<a href="?subtopic=lostaccount&action=step1&action_type=reckey&nick=' .
    urlencode($nick) .
    '" border="0"><IMG SRC="' .
    $template_path .
