...
if (isset($_POST['changeinfosave']) && $_POST['changeinfosave'] == 1) {
  ...
  if (empty($errors)) {
    ...
    $account_logged->logAction(
      "Changed Real Name to <b>$new_rlname</b>, Location to <b>$new_location</b>, Phone to <b>$new_phone</b> and Country to <b>{$config['countries'][$new_country]}</b>."
    );
...
if ($show_form) {
  ...
  $twig->display('account.change_info.html.twig', [
    'countries' => $countries ?? [],
