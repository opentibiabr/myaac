  ...
  if (isset($_POST['registeraccountsave']) && $_POST['registeraccountsave'] == '1') {
    if ($reg_password == $account_logged->getPassword()) {
      if ($coins < $needCoins) {
        ...
        $mailBody = $twig->render('mail.account.register.html.twig', [
        ...
        if (_mail($account_logged->getEMail(), $config['lua']['serverName'] . ' - new recovery key', $mailBody)) {
          ...
          $message =
            '<br /><p class="error">An error occurred while sending email ( <b>' .
            $account_logged->getEMail() .
            '</b> ) with recovery key! Recovery key not changed. Try again later. For Admin: More info can be found in system/logs/mailer-error.log</p>';
        }
