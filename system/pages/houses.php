...
if (isset($_GET['page']) && $_GET['page'] == 'view' && isset($_REQUEST['house'])) {
  ...
  $twig->display('houses.view.html.twig', [
    'errors' => $errors,
...
if (isset($_POST['town']) && isset($_POST['state']) && isset($_POST['order']) && (isset($_POST['type']) || !$db->hasColumn('houses', 'guild'))) {
  ...
  if (!empty($type) && $type != 'all') {
    ...
    if ($db->hasColumn('houses', 'guild')) {
      $guildColumn = 'guild';
    } elseif ($db->hasColumn('houses', 'guildid')) {
      $guildColumn = 'guildid';
    }
  ...
  $players_info = $db->query(
    'SELECT `houses`.`id` AS `houseid` , `players`.`name` AS `ownername` FROM `houses` , `players` , `accounts` WHERE `players`.`id` = `houses`.`owner` AND `accounts`.`id` = `players`.`account_id`'
  );
  $players = [];
  foreach ($players_info->fetchAll() as $player) {
    $players[$player['houseid']] = ['name' => $player['ownername']];
  }

  $houses = [];
  foreach ($houses_info->fetchAll() as $house) {
    $owner = isset($players[$house['id']]) ? $players[$house['id']] : [];
    ...
    $houses[] = ['owner' => $owner, 'name' => $house['name'], 'size' => $house['size'], 'rent' => $house['rent'], 'rentedBy' => $houseRent];
  }
