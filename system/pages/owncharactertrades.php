                                    ...
                                    <table class="TableContent" width="100%" style="border:1px solid #faf0d7;">
                                        ...
                                        if ($logged) {
                                          $getAuctionsbyAccount = $db->query(
                                            "SELECT `id`, `account_old`, `account_new`, `player_id`, `price`, `date_end`, `date_start`, `bid_account`, `bid_price` FROM `myaac_charbazaar` WHERE `account_old` = {$account_logged->getId()}"
                                          );
                                          $i_bg = 0;
                                          foreach ($getAuctionsbyAccount as $Auction) {
                                            ...
                                            $getCharacterbyAccount = $db->query(
                                              "SELECT `id`, `name`, `level` FROM `players` WHERE `id` = {$Auction['player_id']}"
                                            );
