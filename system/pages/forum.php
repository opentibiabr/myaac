...
if (!$logged) {
  echo 'You are not logged in. <a href="?subtopic=accountmanagement&redirect=' .
    BASE_URL .
    urlencode('?subtopic=forum') .
    '">Log in</a> to post on the forum.<br /><br />';
}
...
if ($canEdit) {
  ...
  if (!empty($action)) {
    ...
    } elseif ($action == 'hide_board') {
      Forum::toggleHidden_board($id, $errors);
      $action = '';
    } elseif ($action == 'moveup_board') {
      Forum::move_board($id, -1, $errors);
      $action = '';
    } elseif ($action == 'movedown_board') {
      Forum::move_board($id, 1, $errors);
      $action = '';
    }
  ...
  if (empty($action) || $action == 'edit_board') {
    ...
    $twig->display('forum.add_board.html.twig', [
...
if (empty($action)) {
  $info = $db
    ->query(
      "SELECT `section`, COUNT(`id`) AS 'threads', SUM(`replies`) AS 'replies' FROM `" .
        FORUM_TABLE_PREFIX .
        'forum` WHERE `first_post` = `id` GROUP BY `section`'
    )
    ->fetchAll();
  ...
  $twig->display('forum.boards.html.twig', [
    'boards' => $boards,
...
} elseif (file_exists(PAGES . 'forum/' . $action . '.php')) {
  require PAGES . 'forum/' . $action . '.php';
} else {
