...
if (!in_array($order, ['country', 'name', 'level', 'vocation'])) {
...
if ($db->hasTable('players_online')) {
  ...
  $playersOnline = $db->query(
    'SELECT `accounts`.`country`, `players`.`name`, `players`.`level`, `players`.`vocation`' .
      ...
      '` as `skulltime`, `' .
      $skull_type .
      '` as `skull` FROM `accounts`, `players`, `players_online` WHERE `players`.`id` = `players_online`.`player_id` AND `accounts`.`id` = `players`.`account_id`  ORDER BY ' .
      $order
  );
} else {
  $playersOnline = $db->query(
    'SELECT `accounts`.`country`, `players`.`name`, `players`.`level`, `players`.`vocation`' .
      ...
      '` as `skulltime`, `' .
      $skull_type .
      '` as `skull` FROM `accounts`, `players` WHERE `players`.`online` > 0 AND `accounts`.`id` = `players`.`account_id`  ORDER BY ' .
      $order
  );
}
...
if ($config['online_record']) {
  ...
  if ($db->hasTable('server_record')) {
    $query = $db->query(
      'SELECT `record`, `timestamp` FROM `server_record` WHERE `world_id` = ' . (int) $config['lua']['worldId'] . ' ORDER BY `record` DESC LIMIT 1'
    );
    $timestamp = true;
