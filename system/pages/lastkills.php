...
if ($cache->enabled() && $cache->fetch('last_kills', $tmp)) {
  ...
  if ($db->hasTable('player_killers')) {
    // tfs 0.3
    $players_deaths = $db->query(
      'SELECT `player_deaths`.`id`, `player_deaths`.`date`, `player_deaths`.`level`, `players`.`name`' .
        ($db->hasColumn('players', 'world_id') ? ', `players`.`world_id`' : '') .
        ' FROM `player_deaths` LEFT JOIN `players` ON `player_deaths`.`player_id` = `players`.`id` ORDER BY `date` DESC LIMIT 0, ' .
        $config['last_kills_limit']
    );
    ...
    if (!empty($players_deaths)) {
      foreach ($players_deaths as $death) {
        ...
        $killers = $db
          ->query(
            "SELECT environment_killers.name AS monster_name, players.name AS player_name, players.deleted AS player_exists FROM killers LEFT JOIN environment_killers ON killers.id = environment_killers.kill_id LEFT JOIN player_killers ON killers.id = player_killers.kill_id LEFT JOIN players ON players.id = player_killers.player_id WHERE killers.death_id = '" .
              $death['id'] .
    ...
    $players_deaths = $db->query(
      'SELECT `p`.`name` AS `victim`, `d`.`killed_by` as `killed_by`, `d`.`time` as `time`, `d`.`level`, `d`.`is_player` FROM `player_deaths` as `d` INNER JOIN `players` as `p` ON d.player_id = p.id ORDER BY `time` DESC LIMIT ' .
        $config['last_kills_limit'] .
        ';'
    );
    if (!empty($players_deaths)) {
...
$twig->display('lastkills.html.twig', [
