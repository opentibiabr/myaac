...
if ($getAuctionStep == 'confirm') {
  ...
  if (isset($_POST['auction_confirm']) && isset($_POST['auction_price']) && isset($_POST['auction_days']) && isset($_POST['auction_character'])) {
    ...
    if ($getCoinsAccountLogged['coins'] > $charbazaar_create) {
      ...
      $insert_auction = $db->exec(
        'INSERT INTO `myaac_charbazaar` (`account_old`, `account_new`, `player_id`, `price`, `date_end`, `date_start`) VALUES (' .
          $db->quote($account_old) .
          ', ' .
          $db->quote($account_new) .
          ', ' .
          $db->quote($player_id) .
          ', ' .
          $db->quote($price) .
      ...
      $auctionId = $db->query(
        "SELECT `id` FROM `myaac_charbazaar` WHERE `account_old` = {$account_old} AND `player_id` = {$player_id} ORDER BY `id` DESC LIMIT 1;"
      );
