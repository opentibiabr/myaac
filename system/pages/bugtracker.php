...
if (!$logged) {
  echo 'You are not logged in. <a href="?subtopic=accountmanagement&redirect=' .
    BASE_URL .
    urlencode('?subtopic=bugtracker') .
    '">Log in</a> to post on the bug tracker.<br /><br />';
  return;
}
...
if (admin() and isset($_REQUEST['control']) && $_REQUEST['control'] == 'true') {
  ...
  if (!empty($_REQUEST['id']) and is_numeric($_REQUEST['id']) and !empty($_REQUEST['acc']) and is_numeric($_REQUEST['acc'])) {
    $bug[2] = $db
      ->query(
        'SELECT * FROM ' .
          $db->tableName(TABLE_PREFIX . 'bugtracker') .
          ' where `account` = ' .
          $_REQUEST['acc'] .
          ' and `id` = ' .
          $_REQUEST['id'] .
  ...
  if (!empty($_REQUEST['id']) and is_numeric($_REQUEST['id']) and !empty($_REQUEST['acc']) and is_numeric($_REQUEST['acc'])) {
    ...
    if (!$reply) {
      ...
      echo '<TABLE BORDER=0 CELLSPACING=1 CELLPADDING=4 WIDTH=100%><TR BGCOLOR=' .
        $config['vdarkborder'] .
        '><TD COLSPAN=2 CLASS=white><B>Bug Tracker</B></TD></TR>';
      echo '<TR BGCOLOR="' .
        $dark .
        '"><td width=40%><i><b>Subject</b></i></td><td>' .
        $tags[$bug[2]['tag']] .
        ' ' .
        escapeHtml($bug[2]['subject']) .
        ' ' .
        $value .
        '</td></tr>';
      ...
      foreach ($answers as $answer) {
        ...
        echo '<br><TABLE BORDER=0 CELLSPACING=1 CELLPADDING=4 WIDTH=100%><TR BGCOLOR=' .
          $config['vdarkborder'] .
          '><TD COLSPAN=2 CLASS=white><B>Answer #' .
          escapeHtml($answer['reply']) .
          '</B></TD></TR>';
      ...
      if ($bug[2]['status'] != 3) {
        $reply = $db
          ->query(
            'SELECT MAX(reply) FROM `' .
              TABLE_PREFIX .
              'bugtracker` where `account` = ' .
              $_REQUEST['acc'] .
              ' and `id` = ' .
              $_REQUEST['id'] .
              ' and `type` = 2'
        ...
        $iswho = $db
          ->query(
            'SELECT * FROM `' .
              ...
              ' and `id` = ' .
              $_REQUEST['id'] .
              ' and `type` = 2 order by `reply` desc limit 1'
          )
          ->fetch();
        ...
        if (isset($_POST['finish'])) {
          ...
          if (!empty($error)) {
            ...
            $INSERT = $db->query(
              'INSERT INTO `' .
                TABLE_PREFIX .
                'bugtracker` (`account`,`id`,`text`,`reply`,`type`, `who`) VALUES (' .
                $db->quote($_REQUEST['acc']) .
                ',' .
                $db->quote($_REQUEST['id']) .
                ',' .
                $db->quote($_POST['text']) .
                ',' .
                $db->quote($reply) .
  ...
  if (!$post) {
    echo '<TABLE BORDER=0 CELLSPACING=1 CELLPADDING=4 WIDTH=100%><TR BGCOLOR=' .
      $config['vdarkborder'] .
      '><TD colspan=2 CLASS=white><B>Bug Tracker Admin</B></TD></TR>';
    $i = 1;
    foreach ($bug[1] as $report) {
      ...
      echo '<TR BGCOLOR="' .
        getStyle($i) .
        '"><td width=75%><a href="?subtopic=bugtracker&control=true&id=' .
        $report['id'] .
        '&acc=' .
        $report['account'] .
        '">' .
  ...
  if (!empty($_REQUEST['id']) and $bug[2] != null) {
    ...
    if (!$reply) {
      ...
      if ($bug[2]['status'] != 3) {
        $reply = $db
          ->query('SELECT MAX(reply) FROM `' . TABLE_PREFIX . 'bugtracker` where `account` = ' . $acc . ' and `id` = ' . $id . ' and `type` = 2')
          ->fetch();
        $reply = $reply[0] + 1;
        $iswho = $db
          ->query(
            'SELECT * FROM `myaac_bugtracker` where `account` = ' . $acc . ' and `id` = ' . $id . ' and `type` = 2 order by `reply` desc limit 1'
          )
          ->fetch();
        ...
        if (isset($_POST['finish'])) {
          ...
          if (!empty($error)) {
            ...
            $type = 2;
            $INSERT = $db->query(
              'INSERT INTO `myaac_bugtracker` (`account`,`id`,`text`,`reply`,`type`) VALUES (' .
                $db->quote($acc) .
                ',' .
                $db->quote($id) .
                ',' .
    ...
    echo '<TABLE BORDER=0 CELLSPACING=1 CELLPADDING=4 WIDTH=100%><TR BGCOLOR=' .
      $config['vdarkborder'] .
      '><TD CLASS=white><B>Bug Tracker</B></TD></TR>';
  ...
  if (!$post) {
    if (!isset($_REQUEST['add']) || $_REQUEST['add'] != true) {
      ...
      foreach ($bug[1] as $report) {
        ...
        echo '<TR BGCOLOR="' .
          $bgcolor .
          '"><td width=75%><a href="?subtopic=bugtracker&id=' .
          $report['id'] .
          '">' .
          $tags[$report['tag']] .
          ' ' .
      ...
      $thread = $db
        ->query('SELECT * FROM `' . TABLE_PREFIX . 'bugtracker` where `account` = ' . $acc . ' and `type` = 1 order by `id` desc')
        ->fetch();
      ...
      if (isset($_POST['submit'])) {
        ...
        if (!empty($error)) {
          ...
          $INSERT = $db->query(
            'INSERT INTO `' .
              TABLE_PREFIX .
              'bugtracker` (`account`,`id`,`text`,`type`,`subject`, `reply`,`status`,`tag`) VALUES (' .
              $db->quote($acc) .
              ',' .
              $db->quote($id_next) .
              ',' .
