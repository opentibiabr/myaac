                            ...
                            <td>
                                <div class="TableContentContainer">
                                    <table class="TableContent" width="100%" style="border:1px solid #faf0d7;">
                                        ...
                                        $getBidsbyAccount = [];
                                        if ($logged) {
                                          $getBidsbyAccount = $db->query(
                                            "SELECT `id`, `account_id`, `auction_id`, `bid`, `date` FROM `myaac_charbazaar_bid` WHERE `account_id` = {$account_logged->getId()}"
                                          );
                                          $i_bg = 0;
                                          foreach ($getBidsbyAccount as $BidbyAccount) {
                                            ...
                                            $getAuctionbyBid = $db->query(
                                              "SELECT `id`, `account_old`, `account_new`, `player_id`, `price`, `date_end`, `date_start`, `bid_account`, `bid_price` FROM `myaac_charbazaar` WHERE `id` = {$BidbyAccount['auction_id']}"
                                            );
                                            $getAuctionbyBid = $getAuctionbyBid->fetch();

                                            $getCharacterbyAccount = $db->query(
                                              "SELECT `id`, `name`, `level` FROM `players` WHERE `id` = {$getAuctionbyBid['player_id']}"
                                            );
                                            ...
                                            ?>
                                                <tr bgcolor="<?= getStyle($i_bg) ?>">
                                                    ...
                                                    <td style="color: <?= $bg_DateEnd ?>;"><?= date(
  'd M Y',
  strtotime($getAuctionbyBid['date_end'])
) ?></td>
                                                    <td style="color: <?= $bg_DateEnd ?>;"><img
                                                            src="<?= $template_path ?>/images/premiumfeatures/icon_no.png">
                                                    </td>
                                                    <td style="color: <?= $bg_DateEnd ?>;"><?= number_format(
  $getAuctionbyBid['bid_price'],
  0,
  ',',
  ','
) ?>
