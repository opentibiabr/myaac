...
} elseif ($action == 'step1' && $action_type == '') {
...
if ($action == 'step1' && $action_type == 'no_char') {
  if (empty($email_rcv)) {
    ...
    echo '<BR /><BR /><TABLE CELLSPACING=0 CELLPADDING=0 BORDER=0 WIDTH=100%><TR><TD><div style="text-align:center">
				<a href="?subtopic=lostaccount" border="0"><IMG SRC="' .
      $template_path .
      '/images/global/buttons/sbutton_back.gif" NAME="Back" ALT="Back" BORDER=0 WIDTH=120 HEIGHT=18></a></div>
  ...
  if ($account->getCustomField('email_next') < time()) {
    if ($account->getEMail() == $email_rcv) {
      ...
      if (_mail($account_mail, $config['lua']['serverName'] . ' - Recover your account', $mailBody)) {
        ...
        echo '<br />Details about steps required to recover your account has been sent to <b>' .
          $account_mail .
          '</b>. You should receive this email within 15 minutes. Please check your inbox/spam directory.';
      } else {
    ...
    echo 'Account of e-mail (<b>' .
      $email_rcv .
      '</b>) received e-mail in last ' .
      ceil($config['email_lai_sec_interval'] / 60) .
      ' minutes. You must wait ' .
      $timeleft .
      ' before you can use Lost Account Interface again.';
  }
  ...
  if ($action == 'sendcode') {
        ...
        if ($account->getCustomField('email_next') < time()) {
          if ($account->getEMail() == $email) {
            ...
            $mailBody =
              ...
              ' password.<br/>
					<p>Account name: ' .
              $account->getCustomField('name') .
              '</p>
					<br />
					To do so, please click this link:
					<p><a href="' .
              BASE_URL .
              '?subtopic=lostaccount&action=checkcode&code=' .
              $newcode .
              '&character=' .
              urlencode($nick) .
              '">' .
              ...
              '</a></p>
					<p>or open page: <i>' .
              BASE_URL .
              '?subtopic=lostaccount&action=checkcode</i> and in field "code" write <b>' .
              $newcode .
              '</b></p>
					<br/>
          ...
          $secondsleft = $insec - $minutesleft * 60;
          $timeleft = $minutesleft . ' minutes ' . $secondsleft . ' seconds';
          echo 'Account of selected character (<b>' .
            $nick .
            '</b>) received e-mail in last ' .
            ceil($config['email_lai_sec_interval'] / 60) .
            ' minutes. You must wait ' .
    ...
    echo '<BR /><TABLE CELLSPACING=0 CELLPADDING=0 BORDER=0 WIDTH=100%><TR><TD><div style="text-align:center">
				<a href="?subtopic=lostaccount&action=step1&action_type=email&nick=' .
      urlencode($nick) .
      '" border="0"><IMG SRC="' .
      $template_path .
      '/images/global/buttons/sbutton_back.gif" NAME="Back" ALT="Back" BORDER=0 WIDTH=120 HEIGHT=18></a></div>
    ...
    if (empty($code) || (empty($email_rcv) && empty($character))) {
      echo 'Please enter code from e-mail and name of one character from account. Then press Submit.<BR>
				...
				<TR><TD BGCOLOR="' .
        $config['vdarkborder'] .
        '" class="white"><B>Code & character name</B></TD></TR>
				<TR><TD BGCOLOR="' .
        $config['darkborder'] .
        '">
      ...
      if ($account->isLoaded()) {
        if ($account->getCustomField('email_code') == $code) {
          echo '<script type="text/javascript">
            ...
            $character .
            '">
				<INPUT TYPE=hidden NAME="email_rcv" VALUE="' .
            $email_rcv .
            '">
            ...
            '">
				...
				<TR><TD BGCOLOR="' .
            $config['vdarkborder'] .
            '" class="white"><B>Changing Password</B></TD></TR>
				<TR><TD BGCOLOR="' .
            $config['darkborder'] .
            '">
    ...
    if (!empty($error)) {
      echo '<span style="color: red"><b>' .
        $error .
        '</b></span><br />Please enter code from e-mail and name of one character from account. Then press Submit.<BR>
				<FORM ACTION="?subtopic=lostaccount&action=checkcode" METHOD=post>
				<TABLE CELLSPACING=1 CELLPADDING=4 BORDER=0 WIDTH=100%>
				<TR><TD BGCOLOR="' .
        $config['vdarkborder'] .
    ...
    if (empty($code) || (empty($email_rcv) && empty($character)) || empty($newpassword)) {
      echo '<span style="color: red"><b>Error. Try again.</b></span><br />Please enter code from e-mail and name of one character from account. Then press Submit.<BR>
				...
				<TABLE CELLSPACING=0 CELLPADDING=0 BORDER=0 WIDTH=100%><TR><TD><div style="text-align:center">
				<INPUT TYPE=image NAME="Back" ALT="Back" SRC="' .
        $template_path .
        '/images/global/buttons/sbutton_back.gif" BORDER=0 WIDTH=120 HEIGHT=18></div>
				</TD></TR></FORM></TABLE></TABLE>';
    } else {
      $account = new OTS_Account();
      if (empty($email_rcv)) {
          ...
          if (Validator::password($newpassword)) {
            ...
            echo 'New password to your account is below. Now you can login.<BR>
              ...
              '">
					...
					<TR><TD BGCOLOR="' .
              $config['vdarkborder'] .
              '" class="white"><B>Changed password</B></TD></TR>
					<TR><TD BGCOLOR="' .
              $config['darkborder'] .
              '">
					New password:&nbsp;<b>' .
            ...
            $mailBody =
              '
					...
					<p>Changed password to your account in Lost Account Interface on server <a href="' .
              BASE_URL .
              '"><b>' .
              $config['lua']['serverName'] .
              '</b></a></p>
            ...
            echo '</TD></TR>
              ...
              $template_path .
              '/images/global/buttons/sbutton_login.gif" BORDER=0 WIDTH=120 HEIGHT=18></div>
				</TD></TR></FORM></TABLE></TABLE>';
          } else {
