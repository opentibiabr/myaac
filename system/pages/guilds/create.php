...
if (empty($todo)) {
  if (count($array_of_player_nig) == 0) {
    $guild_errors[] =
      'On your account all characters are in guilds, have too low level to create new guild' .
      ($config['guild_need_premium'] ? ' or you don\' have a premium account' : '') .
      '.';
  }
}
...
if ($todo == 'save') {
  ...
  if (empty($guild_errors)) {
    if ($player->getLevel() < $config['guild_need_level']) {
      $guild_errors[] =
        'Character <b>' . $name . '</b> has too low level. To create guild you need character with level <b>' . $config['guild_need_level'] . '</b>.';
    }
    if ($config['guild_need_premium'] && !$account_logged->isPremium()) {
...
if (!empty($guild_errors)) {
  $twig->display('error_box.html.twig', ['errors' => $guild_errors]);
  unset($todo);
}
...
if (isset($todo) && $todo == 'save') {
  ...
  $twig->display('guilds.create.success.html.twig', [
  ...
  $twig->display('guilds.create.html.twig', [
