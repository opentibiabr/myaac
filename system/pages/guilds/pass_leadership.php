  ...
  if ($logged) {
    ...
    if ($guild_leader) {
      ...
      if ($saved) {
        ...
        $twig->display('guilds.pass_leadership.html.twig', [
          'guild' => $guild,
        ]);
      }
    } else {
...
if (empty($guild_errors) && !empty($guild_errors2)) {
  $twig->display('error_box.html.twig', ['errors' => $guild_errors2]);

  echo '<br/><div style="text-align:center"><form action="?subtopic=guilds&guild=' .
    $guild->getName() .
    '&action=pass_leadership" method="post">' .
    $twig->render('buttons.back.html.twig') .
    '</form></div>';
}
if (!empty($guild_errors)) {
  ...
  echo '<br/><div style="text-align:center"><form action="?subtopic=guilds" method="post">' .
    $twig->render('buttons.back.html.twig') .
    '</form></div>';
}
