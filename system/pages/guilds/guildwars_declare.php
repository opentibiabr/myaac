  ...
  foreach ($account_players as $player) {
    ...
    if ($player_rank->isLoaded()) {
      foreach ($rank_list as $rank_in_guild) {
        if ($guild_owner->isLoaded() && $rank_in_guild->isLoaded() && $player_rank->isLoaded() && $rank_in_guild->getId() == $player_rank->getId()) {
          $players_from_account_in_guild[] = $player->getName();
          if ($guild_owner->getId() == $player->getId()) {
            $guild_vice = true;
            $guild_leader = true;
          } elseif ($player_rank->getLevel() > 1) {
            $guild_vice = true;
    ...
    if ($guild->getName() != $guild_name) {
      ...
      if ($guilds_war_list['guild1'] != $myguild_id && $guilds_war_list['guild2'] != $guild->getId()) {
        if ($guilds_war_list['guild1'] != $guild->getId() && $guilds_war_list['guild2'] != $myguild_id) {
          ...
          $guilds[] = ['id' => $guildId, 'name' => $guildName];
        }
      }
    }
  }
...
if (!empty($errors)) {
  ...
  $twig->display('guilds.guildwars_declare.html.twig', [
    'guilds' => $guilds,
...
if (!empty($errors)) {
  ...
  if (isset($_REQUEST['war_st']) && $_REQUEST['war_st'] == 'save') {
    if (isset($_REQUEST['war_opp']) && $_REQUEST['war_days'] > 7 && $_REQUEST['war_kills'] > 10) {
      ...
      $insert_war = $db->exec(
        "INSERT INTO `guild_wars` (`guild1`, `guild2`, `name1`, `name2`, `status`, `duration`, `kills`, `price`, `comment`) VALUES ('" .
          intval($myguild_id) .
          "', '" .
          intval($opp_id) .
          "', '" .
