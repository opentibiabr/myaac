...
$description_with_lines = str_replace(["\r\n", "\n", "\r"], '<br />', $description, $count);
  ...
  if ($db->hasTable(GUILD_MEMBERS_TABLE)) {
    $players_with_rank = $db->query(
      'SELECT `players`.`id` as `id`, `' .
        GUILD_MEMBERS_TABLE .
        '`.`rank_id` as `rank_id` FROM `players`, `' .
        GUILD_MEMBERS_TABLE .
        '` WHERE `' .
        GUILD_MEMBERS_TABLE .
        '`.`rank_id` = ' .
        ...
        ' AND `players`.`id` = `' .
        GUILD_MEMBERS_TABLE .
        '`.`player_id` ORDER BY `name`;'
    );
  } elseif ($db->hasColumn('players', 'rank_id')) {
...
$twig->display('guilds.view.html.twig', [
  'logo' => $guild_logo,
