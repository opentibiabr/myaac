...
if (empty($errors)) {
  if ($logged) {
    ...
    if ($guild_leader) {
      ...
      $allowed_ext = ['image/gif', 'image/jpg', 'image/pjpeg', 'image/jpeg', 'image/bmp', 'image/png', 'image/x-png'];
      $ext_name = [
        'image/gif' => 'gif',
        'image/jpg' => 'jpg',
        'image/jpeg' => 'jpg',
      ...
      if (isset($_REQUEST['todo']) && $_REQUEST['todo'] == 'save') {
        ...
        if (is_uploaded_file($file['tmp_name'])) {
          if ($file['size'] > $max_image_size_b) {
            $upload_errors[] =
              'Uploaded image is too big. Size: <b>' . $file['size'] . ' bytes</b>, Max. size: <b>' . $max_image_size_b . ' bytes</b>.';
          }

          $type = strtolower($file['type']);
          if (!in_array($type, $allowed_ext)) {
            $upload_errors[] =
              'Your file type isn\' allowed. Allowed: <b>gif, jpg, bmp, png</b>. Your file type: <b>' .
              $type .
              '</b> If it\'s valid image contact with admin.';
          }
        } else {
        ...
        if (empty($upload_errors)) {
          ...
          $guild_logo = str_replace(['..', '/', '\\'], ['', '', ''], $guild->getCustomField('logo_name'));
        ...
        if (!empty($upload_errors)) {
          $twig->display('error_box.html.twig', ['errors' => $upload_errors]);
        } else {
      ...
      $twig->display('guilds.change_logo.html.twig', [
