...
if (Forum::canPost($account_logged)) {
  $players_from_account = $db
    ->query('SELECT `players`.`name`, `players`.`id` FROM `players` WHERE `players`.`account_id` = ' . (int) $account_logged->getId())
    ->fetchAll();
  ...
  if (isset($thread['id']) && Forum::hasAccess($thread['section'])) {
    echo '<div class="ForumBreadCrumbs"><a href="' .
      getLink('forum') .
      '">Community Boards</a> | <a href="' .
      getForumBoardLink($thread['section']) .
      '">' .
      $sections[$thread['section']]['name'] .
      '</a> | <a href="' .
      getForumThreadLink($thread_id) .
      '">' .
      $thread['post_topic'] .
      '</a> | <b>Post New Reply</b></div><br />';
    ...
    if (isset($_REQUEST['quote'])) {
      $quoted_post = $db
        ->query(
          'SELECT `players`.`name`, `' .
            ...
            'forum`.`post_text`, `' .
            FORUM_TABLE_PREFIX .
            'forum`.`post_date` FROM `players`, `' .
            FORUM_TABLE_PREFIX .
            'forum` WHERE `players`.`id` = `' .
            FORUM_TABLE_PREFIX .
            'forum`.`author_guid` AND `' .
            FORUM_TABLE_PREFIX .
            'forum`.`id` = ' .
            (int) $quote
        )
      ...
      if (isset($quoted_post[0]['name'])) {
        $text =
          ...
          $quoted_post[0]['name'] .
          ' on ' .
          date('d.m.y H:i:s', $quoted_post[0]['post_date']) .
          ':[/i][quote]' .
      ...
      if (count($errors) == 0) {
        ...
        if ($last_post + $config['forum_post_interval'] - time() > 0 && !Forum::isModerator()) {
          $errors[] =
            'You can post one time per ' .
            $config['forum_post_interval'] .
            ' seconds. Next post after ' .
            ($last_post + $config['forum_post_interval'] - time()) .
            ' second(s).';
        }
      }
      if (count($errors) == 0) {
        ...
        $post_page = $db
          ->query(
            'SELECT COUNT(`' .
              FORUM_TABLE_PREFIX .
              'forum`.`id`) AS posts_count FROM `players`, `' .
              FORUM_TABLE_PREFIX .
              'forum` WHERE `players`.`id` = `' .
              FORUM_TABLE_PREFIX .
              'forum`.`author_guid` AND `' .
    ...
    if (!$saved) {
      ...
      foreach ($threads as &$thread) {
        ...
        if ($player_account->isLoaded()) {
          $thread['post'] = Forum::showPost(
            $thread['post_html'] > 0 ? $thread['post_topic'] : htmlspecialchars($thread['post_topic']),
            $thread['post_html'] > 0 ? $thread['post_text'] : htmlspecialchars($thread['post_text']),
            $thread['post_smile'] == 0,
            $thread['post_html'] > 0
          );
        }
      ...
      $twig->display('forum.new_post.html.twig', [
        'thread_id' => $thread_id,
  ...
  $errors[] =
    "Your account is banned, deleted or you don't have any player with level " .
    $config['forum_level_required'] .
    " on your account. You can't post.";
  displayErrorBoxWithBackButton($errors, getLink('forum'));
}
