...
$thread_starter = $db
  ->query(
    'SELECT `players`.`name`, `' .
      ...
      'forum`.`id` = `' .
      FORUM_TABLE_PREFIX .
      'forum`.`first_post` AND `players`.`id` = `' .
      FORUM_TABLE_PREFIX .
      'forum`.`author_guid` LIMIT 1'
  )
  ->fetch();
...
$posts = $db
  ->query(
    'SELECT `players`.`id` as `player_id`, `' .
      ...
      ' ORDER BY `' .
      FORUM_TABLE_PREFIX .
      'forum`.`post_date` LIMIT ' .
      $config['forum_posts_per_page'] .
      ' OFFSET ' .
      $_page * $config['forum_posts_per_page']
  )
  ->fetchAll();
...
foreach ($posts as &$post) {
  ...
  if ($config['characters']['outfit']) {
    $post['outfit'] =
      ...
      '?id=' .
      $player->getLookType() .
      ($lookaddons ? '&addons=' . $player->getLookAddons() : '') .
      '&head=' .
      $player->getLookHead() .
      '&body=' .
      $player->getLookBody() .
  ...
  $post['content'] = Forum::showPost(
    $post['post_html'] > 0 ? $post['post_topic'] : htmlspecialchars($post['post_topic']),
    $post['post_html'] > 0 ? $post['post_text'] : htmlspecialchars($post['post_text']),
    $post['post_smile'] == 0,
    $post['post_html'] > 0
  );

  $query = $db
    ->query("SELECT COUNT(`id`) AS 'posts' FROM `" . FORUM_TABLE_PREFIX . 'forum` WHERE `author_aid`=' . (int) $player_account->getId())
    ->fetch();
...
$twig->display('forum.show_thread.html.twig', [
  'thread_id' => $thread_id,
  'posts' => $posts,
  'links_to_pages' => $links_to_pages,
  'author_link' => getPlayerLink($thread_starter['name']),
  'section' => ['id' => $posts[0]['section'], 'name' => $sections[$posts[0]['section']]['name']],
