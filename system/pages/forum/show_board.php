...
$threads_count = $db
  ->query(
    'SELECT COUNT(`' .
      FORUM_TABLE_PREFIX .
      'forum`.`id`) AS threads_count FROM `players`, `' .
      FORUM_TABLE_PREFIX .
                                    ...
                                    <table class="TableContent" width="100%" style="border:1px solid #faf0d7;">
                                        ...
                                        $last_threads = $db
                                          ->query(
                                            'SELECT `players`.`id` as `player_id`, `players`.`name`, `' .
                                              ...
                                              'forum`.`id` ORDER BY `' .
                                              FORUM_TABLE_PREFIX .
                                              'forum`.`last_post` DESC LIMIT ' .
                                              $config['forum_threads_per_page'] .
                                              ' OFFSET ' .
                                              $_page * $config['forum_threads_per_page']
                                          )
                                          ->fetchAll();
                                        ...
                                        if (isset($last_threads[0])) {
                                          ...
                                          foreach ($last_threads as $thread) {
                                            ...
                                            echo '<a href="' .
                                              getForumThreadLink($thread['id']) .
                                              '">' .
                                              ($canEditForum ? $thread['post_topic'] : htmlspecialchars($thread['post_topic'])) .
                                              '</a>
		<br /><small>' .
                                              ($canEditForum
                                                ? substr(strip_tags($thread['post_text']), 0, 50)
                                                : htmlspecialchars(substr($thread['post_text'], 0, 50))) .
                                              '...</small></td><td>' .
                                              ...
                                              $last_post = $db
                                                ->query(
                                                  'SELECT `players`.`name`, `' .
                                                    ...
                                                    ' AND `players`.`id` = `' .
                                                    FORUM_TABLE_PREFIX .
                                                    'forum`.`author_guid` ORDER BY `post_date` DESC LIMIT 1'
                                                )
                                                ->fetch();
                                              if (isset($last_post['name'])) {
                                                echo '<img src="' .
                                                  $template_path .
                                                  '/images/global/forum/logo_lastpost.gif" border="0" width="10" height="9"> ' .
                                                  date('d.m.y H:i:s', $last_post['post_date']) .
                                                  '<br />by ' .
                                                  getPlayerLink($last_post['name']);
                                              } else {
                                                echo 'No posts.';
                                              }
                                            } else {
                        ...
                        <tr>
                            <td><?php echo 'Page: ' . $links_to_pages . '<br />'; ?></td>
                        </tr>
                        </tbody>
                    </table>
                </div>
