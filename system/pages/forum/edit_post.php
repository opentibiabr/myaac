...
if (Forum::canPost($account_logged)) {
  ...
  $thread = $db
    ->query(
      'SELECT `author_guid`, `author_aid`, `first_post`, `post_topic`, `post_date`, `post_text`, `post_smile`, `post_html`, `id`, `section` FROM `' .
        FORUM_TABLE_PREFIX .
        'forum` WHERE `id` = ' .
        $post_id .
        ' LIMIT 1'
  ...
  if (isset($thread['id'])) {
    ...
    echo '<a href="' .
      ...
      '</a> >> <a href="' .
      getForumThreadLink($thread['first_post']) .
      '">' .
      $first_post['post_topic'] .
      '</a> >> <b>Edit post</b>';
    if (Forum::hasAccess($thread['section'] && ($account_logged->getId() == $thread['author_aid'] || Forum::isModerator()))) {
      ...
      if (isset($_REQUEST['save'])) {
        ...
        if (count($errors) == 0) {
          ...
          echo '<br />Thank you for editing post.<br /><a href="' .
            getForumThreadLink($thread['first_post'], $_page) .
            '">GO BACK TO LAST THREAD</a>';
        }
      } else {
      ...
      if (!$saved) {
        ...
        $twig->display('forum.edit_post.html.twig', [
          'post_id' => $post_id,
