...
if ($canEdit) {
  if (function_exists('imagecreatefrompng')) {
    ...
    if (!isset($_GET['image'])) {
      $twig->display('gallery.form.html.twig', [
...
if (isset($_GET['image'])) {
  ...
  $previous_image = $db->query(
    'SELECT `id` FROM `' . TABLE_PREFIX . 'gallery` WHERE `id` = ' . $db->quote($image['id'] - 1) . ' ORDER by `ordering`;'
  );
  ...
  $twig->display('gallery.get.html.twig', [
...
$images = $db->query(
  'SELECT `id`, `comment`, `image`, `author`, `thumb`' .
...
class Gallery
{
  public static function add($comment, $image, $author, &$errors)
  {
    global $db;
    if (isset($comment[0]) && isset($image[0]) && isset($author[0])) {
      $query = $db->query('SELECT `ordering`' . ' FROM `' . TABLE_PREFIX . 'gallery`' . ' ORDER BY `ordering`' . ' DESC LIMIT 1');
      ...
      if (
        $db->insert(TABLE_PREFIX . 'gallery', [
          'comment' => $comment,
          'image' => $filename,
          'author' => $author,
  ...
  {
    global $db;
    return $db->select(TABLE_PREFIX . 'gallery', ['id' => $id]);
  }

  public static function update($id, $comment, $image, $author)
  {
    global $db;
  ...
  public static function delete($id, &$errors)
  {
    ...
    if (isset($id)) {
      if (self::get($id) !== false) {
        $db->delete(TABLE_PREFIX . 'gallery', ['id' => $id]);
      } else {
  ...
  public static function toggleHidden($id, &$errors)
  {
    ...
    if (isset($id)) {
      $query = self::get($id);
      if ($query !== false) {
        $db->update(TABLE_PREFIX . 'gallery', ['hidden' => $query['hidden'] == 1 ? 0 : 1], ['id' => $id]);
      } else {
        $errors[] = 'Image with id ' . $id . ' does not exists.';
      }
    } else {
  ...
  public static function move($id, $i, &$errors)
  {
    ...
    if ($query !== false) {
      ...
      $old_record = $db->select(TABLE_PREFIX . 'gallery', ['ordering' => $ordering]);
      if ($old_record !== false) {
        $db->update(TABLE_PREFIX . 'gallery', ['ordering' => $query['ordering']], ['ordering' => $ordering]);
      }

      $db->update(TABLE_PREFIX . 'gallery', ['ordering' => $ordering], ['id' => $id]);
    } else {
      $errors[] = 'Image with id ' . $id . ' does not exists.';
    }
  ...
  public static function resize($file, $new_width, $new_height, $new_file, &$errors)
  {
  ...
  public static function generateThumb($id, $file, &$errors)
  {
    $pathinfo = pathinfo($file);
    $extension = strtolower($pathinfo['extension']);
    $thumb_filename = 'images/gallery/' . $pathinfo['filename'] . '_thumb.' . $extension;
    ...
    if (isset($id)) {
      $query = self::get($id);
      if ($query !== false) {
        $db->update(TABLE_PREFIX . 'gallery', ['thumb' => $thumb_filename], ['id' => $id]);
      } else {
        $errors[] = 'Image with id ' . $id . ' does not exists.';
      }
    } else {
