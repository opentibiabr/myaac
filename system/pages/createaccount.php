...
if ($save) {
  ...
  } elseif (!Validator::accountName($account_name)) {
    $errors['account'] = Validator::getLastError();
  }
  ...
  if ($config['recaptcha_enabled']) {
    if (isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])) {
      $verifyResponse = file_get_contents(
        'https://www.google.com/recaptcha/api/siteverify?secret=' . $config['recaptcha_secret_key'] . '&response=' . $_POST['g-recaptcha-response']
      );
  ...
  if (empty($errors)) {
    ...
    if ($config['mail_enabled'] && $config['account_mail_verify']) {
      ...
      if (_mail($email, 'New account on ' . $config['lua']['serverName'], $body_html)) {
        ...
        $twig->display('success.html.twig', [
          ...
          'description' =>
            ...
            ' is <b>' .
            $tmp_account .
            '</b><br/>You will need the account ' .
            $account_type .
            ' and your password to play on ' .
            ...
            $account_type .
            ' or password to anybody.',
          'custom_buttons' => config('account_create_character_create') ? '' : null,
        ]);
      } else {
        error(
          'An error occurred while sending email! Account not created. Try again. For Admin: More info can be found in system/logs/mailer-error.log'
        );
        $new_account->delete();
      }
    } else {
      ...
      if ($config['mail_enabled'] && $config['account_welcome_mail']) {
        $mailBody = $twig->render('account.welcome_mail.html.twig', [
          'account' => $tmp_account,
          'email' => $email,
