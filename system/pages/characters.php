 ...
 */<?php
 ...
 {
   global $config, $twig;
   return $twig->render('characters.form.html.twig', ['link' => getLink('characters'), 'autofocus' => $autofocus]);
 }
 ...
 if ($player->isLoaded() && !$player->isDeleted()) {
   ...
   if ($config['characters']['outfit']) {
     $outfit =
       ...
       '?id=' .
       $player->getLookType() .
       ($db->hasColumn('players', 'lookaddons') ? '&addons=' . $player->getLookAddons() : '') .
       '&head=' .
       $player->getLookHead() .
       '&body=' .
       $player->getLookBody() .
   ...
   if ($marriage_id > 0) {
     ...
     $marriage->load($player->getMarriage(), ['id', 'name'], false);
   ...
   if ($db->hasColumn('houses', 'town_id')) {
     $town_field = 'town_id';
   } elseif ($db->hasColumn('houses', 'townid')) {
     $town_field = 'townid';
   } elseif (!$db->hasColumn('houses', 'town')) {
     $town_field = false;
   }
   if ($db->hasColumn('houses', 'name')) {
     $house = $db
       ->query(
         'SELECT `id`, `paid`, `name`' .
           ($town_field != false ? ', `' . $town_field . '` as `town`' : '') .
           ' FROM `houses` WHERE `owner` = ' .
           $player->getId()
       )
       ->fetch();
     if (isset($house['id'])) {
   ...
   if ($config['characters']['skills']) {
     if ($db->hasColumn('players', 'skill_fist')) {
       ...
       $skills_db = $db
         ->query(
           'SELECT `maglevel`, `skill_fist`, `skill_club`, `skill_sword`, `skill_axe`, `skill_dist`, `skill_shielding`, `skill_fishing` FROM `players` WHERE `id` = ' .
             $player->getId()
         )
         ->fetch();
       $skill_ids = [
       ...
       foreach ($skill_ids as $skillid => $field_name) {
         $skills[] = ['skillid' => $skillid, 'value' => $skills_db[$field_name]];
       }
     } else {
   ...
   if ($quests_enabled) {
     ...
     $storage_sql = $db->query(
       'SELECT `key`, `value` FROM `player_storage` WHERE `player_id` = ' . $player->getId() . ' AND `key` IN (' . $sql_query_in . ')'
     );
     $player_storage = [];
   ...
   if ($db->hasTable('killers')) {
     $player_deaths = $db
       ->query('SELECT `id`, `date`, `level` FROM `player_deaths` WHERE `player_id` = ' . $player->getId() . ' ORDER BY `date` DESC LIMIT 0,10;')
       ->fetchAll();
     if (count($player_deaths)) {
       ...
       foreach ($player_deaths as $death) {
         $killers = $db
           ->query(
             "SELECT environment_killers.name AS monster_name, players.name AS player_name, players.deleted AS player_exists FROM killers LEFT JOIN environment_killers ON killers.id = environment_killers.kill_id
LEFT JOIN player_killers ON killers.id = player_killers.kill_id LEFT JOIN players ON players.id = player_killers.player_id
WHERE killers.death_id = '" .
               $death['id'] .
               "' ORDER BY killers.final_hit DESC, killers.id ASC"
           )
           ->fetchAll();
         $description = '';
         $i = 0;
         ...
         $deaths[] = ['time' => $death['date'], 'description' => $description . '.'];
       }
     }
   } elseif ($db->hasTableAndColumns('player_deaths', ['time', 'level', 'killed_by', 'is_player'])) {
     $mostdamage = '';
     ...
     if (count($deaths_db)) {
       ...
       foreach ($deaths_db as $death) {
         ...
         $deaths[] = ['time' => $death['time'], 'description' => $description];
       }
     }
   }
   $frags = [];
   ...
   if ($config['characters']['frags'] && $db->hasTable('killers')) {
     ...
     $player_frags = $db
       ->query(
         'SELECT `player_deaths`.*, `players`.`name`, `killers`.`unjustified` FROM `player_deaths` LEFT JOIN `killers` ON `killers`.`death_id` = `player_deaths`.`id` LEFT JOIN `player_killers` ON `player_killers`.`kill_id` = `killers`.`id` LEFT JOIN `players` ON `players`.`id` = `player_deaths`.`player_id` WHERE `player_killers`.`player_id` = ' .
           $player->getId() .
     ...
     if (count($player_frags)) {
       ...
       foreach ($player_frags as $frag) {
         ...
         $frags[] = ['time' => $frag['date'], 'description' => $description, 'unjustified' => $frag['unjustified'] != 0];
       }
     }
   } // signature
   ...
   if (!$hidden) {
     ...
     if ($db->hasTable('account_bans')) {
       $banned = $db->query(
         'SELECT `expires_at` as `expires` FROM `account_bans` WHERE `account_id` = ' .
           $account->getId() .
           ' and (`expires_at` > ' .
           time() .
           ' OR `expires_at` = -1);'
       );
     } elseif ($db->hasTable('bans')) {
       if ($db->hasColumn('bans', 'expires')) {
         $banned = $db->query(
           'SELECT `expires` FROM `bans` WHERE (`value` = ' .
             $account->getId() .
             ' or `value` = ' .
             $player->getId() .
             ') and `active` = 1 and `type` != 2 and `type` != 4 and (`expires` > ' .
             time() .
             ' OR `expires` = -1);'
         );
       } else {
         $banned = $db->query(
           'SELECT `time` as `time` FROM `bans` WHERE (`account` = ' .
             $account->getId() .
             ' or `player` = ' .
             $player->getId() .
             ') and `type` != 2 and `type` != 4 and (`time` > ' .
             time() .
             ' OR `time` = -1);'
         );
       }
     ...
     foreach ($query as $p) {
       ...
       $fields = ['id', 'name', 'vocation', 'level', 'online', 'deleted', 'hidden'];
   ...
   foreach ($achievements as $achievement => $value) {
     ...
     $achievementsPlayer = $db
       ->query("SELECT `key`, `value` FROM `player_storage` WHERE `key` = {$achievementStorage} AND `player_id` = {$player->getId()}")
       ->fetch();
   ...
   $twig->display('characters.html.twig', [
     ...
     'guild' => ['rank' => isset($guild_name) ? $rank_of_player->getName() : null, 'link' => isset($guild_name) ? getGuildLink($guild_name) : null],
   ...
   $query = $db->query(
     'SELECT `name`, `level`, `vocation`' .
       ...
       $deleted .
       ' != 1 LIMIT ' .
       (int) config('characters_search_limit') .
       ';'
   );
   if ($query->rowCount() > 0) {
