...
if (ACTION == 'logout' && !isset($_REQUEST['account_login'])) {
  if (!defined('HOOK_LOGOUT_DISPLAY') || HOOK_LOGOUT_DISPLAY) {
    // plugin will take care of this message
...
if ($action == '') {
  ...
  if (empty($recovery_key)) {
    ...
    if ($config['generate_new_reckey'] && $config['mail_enabled']) {
      $account_registered =
        '<b><span style="color: green">Yes ( <a href="' . getLink('account/register/new') . '"> Buy new Recovery Key </a> )</span></b>';
    } else {
      $account_registered =
        '<b><span style="color: green"><img src="' . $template_path . '/images/premiumfeatures/icon_yes.png"> Registered.</span></b>';
    }
  }
  ...
  if ($account_logged->isBanned()) {
    if ($account_logged->getBanTime() > 0) {
      $welcome_message =
        '<span style="color: red">Your account is banished until ' . date('j F Y, G:i:s', $account_logged->getBanTime()) . '!</span>';
    } else {
  ...
  foreach ($account_logged->getActionsLog(0, 1000) as $action) {
    $actions[] = [
      ...
      'date' => $action['date'],
      'ip' => $action['ip'] != 0 ? long2ip($action['ip']) : inet_ntop($action['ipv6']),
    ];
  }
  ...
  } elseif (file_exists(PAGES . 'account/' . $action . '.php')) {
    require PAGES . 'account/' . $action . '.php';
  } else {
