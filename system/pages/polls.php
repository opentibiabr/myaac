...
if (empty($_REQUEST['id']) and !isset($_REQUEST['control']) || $_REQUEST['control'] != 'true') {
  ...
  $closed = $db->query('SELECT * FROM `myaac_polls` where `end` < ' . $time . ' order by `end` desc');
...
if (!$logged) {
  echo 'You are not logged in. <a href="?subtopic=accountmanagement&redirect=' .
    BASE_URL .
    urlencode('?subtopic=polls') .
    '">Log in</a> to vote in polls.<br /><br />';
  return;
}
...
if (!empty($_REQUEST['id']) and !isset($_REQUEST['control']) || $_REQUEST['control'] != 'true') {
  foreach ($POLLS as $POLL) {
    if ($_REQUEST['id'] == $POLL['id']) {
      ...
      $ANSWERS = $db->query(
        'SELECT * FROM ' .
          $db->tableName('myaac_polls_answers') .
          ' where `poll_id` = ' .
          addslashes(htmlspecialchars(trim($_REQUEST['id']))) .
          ' order by `answer_id`'
      );
...
if ($POLL['end'] > $time) {
  ...
  if (isset($_REQUEST['vote']) && $_REQUEST['vote'] == true and $allow == true) {
    if ($account_logged->getCustomField('vote') < $_REQUEST['id'] and !empty($_POST['answer'])) {
      if (isset($_POST['continue'])) {
        ...
        $UPDATE_poll = $db->query(
          'UPDATE `myaac_polls` SET `votes_all` = `votes_all` + 1 where `id` = ' . addslashes(htmlspecialchars(trim($_REQUEST['id']))) . ''
        );
        $UPDATE_answer = $db->query(
          'UPDATE `myaac_polls_answers` SET `votes` = `votes` + 1 where `answer_id` = ' .
            addslashes(htmlspecialchars($_POST['answer'])) .
            ' and`poll_id` = ' .
            addslashes(htmlspecialchars(trim($_REQUEST['id']))) .
            ''
        );
  ...
  if ($account_logged->getCustomField('vote') < $_REQUEST['id'] and $allow == true) {
    echo '<TABLE BORDER=0 CELLSPACING=1 CELLPADDING=4 WIDTH=100%><TR BGCOLOR=' .
      $config['vdarkborder'] .
      '><TD COLSPAN=2 class=white><B>Vote</B></TD></TR>';
    echo '<TR BGCOLOR="' .
      $dark .
      '"><td COLSPAN=2><b>' .
      $POLL['question'] .
    ...
    $ANSWERS_input = $db->query(
      'SELECT * FROM ' . $db->tableName('myaac_polls_answers') . ' where `poll_id` = ' . $_REQUEST['id'] . ' order by `answer_id`'
    );
    $i = 1;
