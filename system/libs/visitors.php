...
class Visitors
{
  ...
  public function visitorExists($ip)
  {
    ...
    global $db;
    $users = $db
      ->query('SELECT COUNT(`ip`) as count FROM `' . TABLE_PREFIX . 'visitors' . '` WHERE ' . $db->fieldName('ip') . ' = ' . $db->quote($ip))
      ->fetch();
  ...
  private function cleanVisitors()
  {
    ...
    $db->exec(
      'DELETE FROM ' .
        $db->tableName(TABLE_PREFIX . 'visitors') .
        ' WHERE ' .
        $db->fieldName('lastvisit') .
        ' < ' .
        (time() - $this->sessionTime * 60)
    );
  }
  ...
  {
    if ($this->cacheEnabled) {
      $this->data[$ip] = ['page' => $page, 'lastvisit' => time()];
      return;
    }
