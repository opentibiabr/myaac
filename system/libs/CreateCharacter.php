  ...
  public function doCreate($name, $sex, $vocation, $town, $account, &$errors)
  {
    ...
    if (empty($errors)) {
      ...
      if ($number_of_players_on_account >= config('characters_per_account')) {
        $errors[] =
          'You have too many characters on your account <b>(' . $number_of_players_on_account . '/' . config('characters_per_account') . ')</b>!';
      }
    }
    ...
    if (empty($errors)) {
      ...
      if (!$char_to_copy->isLoaded()) {
        $errors[] =
          'Wrong characters configuration. Try again or contact with admin. ADMIN: Edit file config.php and set valid characters to copy names. Character to copy: <b>' .
          $char_to_copy_name .
          '</b> doesn\'t exist.';
      }
    }
    ...
    for ($skill = POT::SKILL_FIRST; $skill <= POT::SKILL_LAST; $skill++) {
      $player->setSkill($skill, config('use_character_sample_skills') ? $char_to_copy->getSkill($skill) : 10);
    }
    ...
    if ($db->hasTableAndColumns('player_items', ['pid', 'sid', 'itemtype'])) {
      ...
      foreach ($loaded_items_to_copy as $save_item) {
        $db->query(
          "INSERT INTO `player_items` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `attributes`) VALUES ({$player->getId()}, {$db->quote(
            $save_item['pid']
          )}, {$db->quote($save_item['sid'])}, {$db->quote($save_item['itemtype'])}, {$db->quote($save_item['count'])}, {$db->quote(
