...
class Validator
{
  ...
  public static function email($email)
  {
    ...
    if (
      !preg_match(
        '/^(?:[\w\!\#\$\%\&\'\*\+\-\/\=\?\^\`\{\|\}\~]+\.)*[\w\!\#\$\%\&\'\*\+\-\/\=\?\^\`\{\|\}\~]+@(?:(?:(?:[a-zA-Z0-9_](?:[A-z0-9_\-](?!\.)){0,61}[a-zA-Z0-9_]?\.)+[a-zA-Z0-9_](?:[a-zA-Z0-9_\-](?!$)){0,61}[a-zA-Z0-9_]?)|(?:\[(?:(?:[01]?\d{1,2}|2[0-4]\d|25[0-5])\.){3}(?:[01]?\d{1,2}|2[0-4]\d|25[0-5])\]))$/',
        $email
      )
    ) {
  ...
  public static function newCharacterName($name)
  {
    ...
    $words_blocked = array_merge($custom_words_blocked, [
      'admin',
      'administrator',
      'gamemaster',
      'game master',
    ...
    for ($i = 0; $i < $name_length; $i++) {
      if (
        isset($name_lower[$i]) &&
        isset($name_lower[$i + 1]) &&
        $name_lower[$i] == $name_lower[$i + 1] &&
        isset($name_lower[$i + 2]) &&
        $name_lower[$i] == $name_lower[$i + 2]
      ) {
