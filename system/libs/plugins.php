...
class Plugins
{
  ...
  public static function install($file)
  {
    ...
    if (!$zip->extractTo($plugin_temp_dir)) {
      // place in cache dir
    ...
    if ($plugin_json == null) {
      ...
      if (isset($plugin_json['require'])) {
        ...
        if (!$myaac_satified) {
          self::$error =
            "Your AAC version doesn't meet the requirement of this plugin. Required version is: " .
            $require_myaac .
        ...
        if (!$php_satisfied) {
          self::$error =
            "Your PHP version doesn't meet the requirement of this plugin. Required version is: " .
            $require_php .
            ", and you're using version " .
            phpversion() .
            '.';
        ...
        if (!$database_satisfied) {
          self::$error =
            "Your database version doesn't meet the requirement of this plugin. Required version is: " .
            $require_database .
            ", and you're using version " .
            DATABASE_VERSION .
            '.';
        ...
        if ($continue) {
          foreach ($require as $req => $version) {
            ...
            if (in_array($req, ['myaac', 'myaac_', 'php', 'php_', 'database', 'database_'])) {
              continue;
            }

            if (in_array($req, ['php-ext', 'php-extension'])) {
            ...
            } elseif (!self::is_installed($req, $version)) {
              self::$error =
                'This plugin requires another plugin to run correctly. The another plugin is: ' . $req . ', with version ' . $version . '.';
              $continue = false;
              break;
            }
          }
