...
if ($configForumTablePrefix !== null && !empty(trim($configForumTablePrefix))) {
  if (!in_array($configForumTablePrefix, ['myaac_', 'z_'])) {
...
class Forum
{
    ...
    return $db
      ->query(
        'SELECT `id` FROM `players` WHERE `account_id` = ' .
          $db->quote($account->getId()) .
          ' AND `level` >= ' .
          $db->quote($config['forum_level_required']) .
          ' LIMIT 1'
      )
      ->rowCount() > 0;
  }
  ...
  public static function add_board($name, $description, $access, $guild, &$errors)
  {
    ...
    if (isset($name[0]) && isset($description[0])) {
      $query = $db->select(TABLE_PREFIX . 'forum_boards', ['name' => $name]);
      ...
      if ($query === false) {
        $query = $db->query(
          'SELECT ' .
            ...
            ' FROM ' .
            $db->tableName(TABLE_PREFIX . 'forum_boards') .
            ' ORDER BY ' .
            $db->fieldName('ordering') .
            ' DESC LIMIT 1'
        );
        ...
        $db->insert(TABLE_PREFIX . 'forum_boards', [
          'name' => $name,
  ...
  {
    global $db;
    return $db->select(TABLE_PREFIX . 'forum_boards', ['id' => $id]);
  }
  ...
  public static function update_board($id, $name, $access, $guild, $description)
  {
    global $db;
    $db->update(
      TABLE_PREFIX . 'forum_boards',
      ['name' => $name, 'description' => $description, 'access' => $access, 'guild' => $guild],
      ['id' => $id]
    );
  }
  ...
  public static function delete_board($id, &$errors)
  {
    ...
    if (isset($id)) {
      if (self::get_board($id) !== false) {
        $db->delete(TABLE_PREFIX . 'forum_boards', ['id' => $id]);
      } else {
  ...
  public static function toggleHidden_board($id, &$errors)
  {
    ...
    if (isset($id)) {
      ...
      if ($query !== false) {
        $db->update(TABLE_PREFIX . 'forum_boards', ['hidden' => $query['hidden'] == 1 ? 0 : 1], ['id' => $id]);
      } else {
        $errors[] = 'Forum board with id ' . $id . ' does not exists.';
      }
    } else {
  ...
  public static function move_board($id, $i, &$errors)
  {
    ...
    if ($query !== false) {
      ...
      $old_record = $db->select(TABLE_PREFIX . 'forum_boards', ['ordering' => $ordering]);
      if ($old_record !== false) {
        $db->update(TABLE_PREFIX . 'forum_boards', ['ordering' => $query['ordering']], ['ordering' => $ordering]);
      }

      $db->update(TABLE_PREFIX . 'forum_boards', ['ordering' => $ordering], ['id' => $id]);
    } else {
      $errors[] = 'Forum board with id ' . $id . ' does not exists.';
    }
  ...
  public static function parseBBCode($text, $smiles)
  {
    ...
    while (stripos($text, '[code]') !== false && stripos($text, '[/code]') !== false) {
      ...
      $text = str_ireplace(
        '[code]' . $code . '[/code]',
        '<i>Code:</i><br /><table cellpadding="0" style="background-color: #' .
          $bgcolor .
          '; width: 480px; border-style: dotted; border-color: #CCCCCC; border-width: 2px"><tr><td>' .
          $code .
          '</td></tr></table>',
        $text
      );
    ...
    while (stripos($text, '[quote]') !== false && stripos($text, '[/quote]') !== false) {
      ...
      $text = str_ireplace(
        '[quote]' . $quote . '[/quote]',
        '<table cellpadding="0" style="background-color: #' .
          $bgcolor .
          '; width: 480px; border-style: dotted; border-color: #007900; border-width: 2px"><tr><td>' .
          $quote .
          '</td></tr></table>',
        $text
      );
    ...
    $tags = [
      ...
      '#\[img\](.*?)\[/img\]#si' => $xhtml
        ? '<img class="forum-image" style="max-width:550px; max-height; 550px;" src="\\1" border="0" alt="" />'
        : '<img class="forum-image" style="max-width:550px; max-height; 550px;" src="\\1" border="0" alt="">',
