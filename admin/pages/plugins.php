...
if (isset($_REQUEST['uninstall'])) {
...
} elseif (isset($_FILES['plugin']['name'])) {
  $file = $_FILES['plugin'];
  $filename = $file['name'];
  ...
  if (isset($file['error'])) {
    ...
    switch ($file['error']) {
      ...
      case UPLOAD_ERR_FORM_SIZE:
        $error .=
          ' - file too large (limit of ' .
          ini_get('upload_max_filesize') .
          ' bytes). You can enlarge the limits by changing "upload_max_filesize" in php.ini';
        break;
  ...
  if (isset($error) && $error != false) {
    ...
    if (is_uploaded_file($file['tmp_name'])) {
      ...
      if ($filetype == 'zip') {
        // check if it is zipped/compressed file
...
$twig->display('admin.plugins.html.twig', [
  'plugins' => $plugins,
]);
