...
if (isset($_REQUEST['template'])) {
  ...
  $menus_db = $db
    ->query(
      'SELECT `name`, `link`, `blank`, `color`, `category`, `ordering` FROM `' .
        TABLE_PREFIX .
        'menu` WHERE `enabled` = 1 AND `template` = ' .
        $db->quote($template) .
  ...
  foreach ($config['menu_categories'] as $id => $cat) {
    echo '        <div class="col-6">
            <div class="box box-danger">
                <div class="box-header with-border">
                    <h3 class="box-title">' .
      $cat['name'] .
      ' <i class="fa fa-plus-circle btn btn-success add-button" title="New" id="add-button-' .
      $id .
      '"></i></h3>
                </div>
                <div class="box-body">';
    ...
    if (isset($menus[$id])) {
      ...
      foreach ($menus[$id] as $menu) {
        echo '<li class="ui-state-default" id="list-' .
          $id .
          '-' .
          $i .
          '"><label>Name: </label><input type="text" name="menu[' .
          $id .
          '][]" value="' .
          escapeHtml($menu['name']) .
          '"/>
				<label>Link: </label><input type="text" name="menu_link[' .
          $id .
          '][]" value="' .
          $menu['link'] .
          '"/>
				<input type="hidden" name="menu_blank[' .
          $id .
          '][]" value="0" />
				<label><input class="blank-checkbox" type="checkbox" ' .
          ($menu['blank'] == 1 ? 'checked' : '') .
          '/> <span title="Open in New Window">Open in New Window</span></label>

				<input class="color-picker" type="text" name="menu_color[' .
          $id .
          '][]" value="#' .
          $menu['color'] .
          '" />

				<a class="btn btn-danger" id="remove-button-' .
          $id .
          '-' .
          $i .
          '"><i class="fas fa-trash-alt text-white" title="Remove"></i></a></li>';
...
';
  ...
  echo '<input type="button" class="btn btn-info pull-right" value="Cancel" onclick="window.location = \'' .
    ADMIN_URL .
    '?p=menus&template=' .
    $template .
    '\';">';
  ...
  $twig->display('admin.menus.js.html.twig', [
    'menus' => $menus,
    'last_id' => $last_id,
  ]);
  ?>
  ...
  $twig->display('admin.menus.form.html.twig', [
    'templates' => $templates,
  ]);
}
