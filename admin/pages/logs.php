  ...
  foreach (scandir($server_path_logs, SCANDIR_SORT_ASCENDING) as $f) {
    ...
    if (is_dir($server_path_logs . $f)) {
      foreach (scandir($server_path_logs . $f, SCANDIR_SORT_ASCENDING) as $f2) {
        ...
        $files[] = [$f . '/' . $f2, $server_path_logs];
      }

      continue;
    }

    $files[] = [$f, $server_path_logs];
  }
}
...
$twig->display('admin.logs.html.twig', ['files' => $files]);
...
if (!empty($file)) {
  if (!preg_match('/[^A-z0-9\' _\/\-\.]/', $file)) {
    if (file_exists($aac_path_logs . $file)) {
      $exist = EXIST_AAC_LOG;
    } elseif (file_exists($server_path_logs . $file)) {
    ...
    if ($exist !== EXIST_NONE) {
      ...
      $twig->display('admin.logs.view.html.twig', ['file' => $file, 'content' => $content]);
    }
  } else {
