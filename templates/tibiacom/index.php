        ...
        <?php if (PAGE !== 'news') {
          if (strpos(URI, 'subtopic=') !== false) {
            ...
            if (URI !== 'account/create' && URI !== 'account/lost' && isset($exp[1])) {
              if ($exp[0] === 'account') {
                $tmp = 'accountmanage';
              } elseif ($exp[0] === 'news' && $exp[1] === 'archive') {
                $tmp = 'newsarchive';
              } else {
                $tmp = $exp[0];
              }
    ...
    <div id="Bodycontainer">
        <div id="ContentRow">
                    ...
                    <div id="LoginButtonContainer"
                        ...
                        <div id="LoginButton"
                            ...
                            <div onClick="LoginButtonAction();" onMouseOver="MouseOverBigButton('LoginButtonOver');"
                                ...
                                <div id="ButtonText" <?= !$logged
                                  ? "style='background-image:url(\"$template_path/images/global/buttons/mediumbutton_login.png\")'"
                                  : '' ?>></div>
                            </div>
                        </div>
                    ...
                    foreach ($config['menu_categories'] as $id => $cat) {
                      if (!isset($menus[$id]) || ($id == MENU_CATEGORY_SHOP && !$config['gifts_system'])) {
                        ...
                        <div id='<?= $cat['id'] ?>' class='menuitem'>
                            ...
                            <?php if ($id == MENU_CATEGORY_SHOP || (!$config['gifts_system'] && $id == MENU_CATEGORY_SHOP - 1)) { ?>
                                <div id='MenuBottom'
                                     style='background-image:url(<?= $template_path ?>/images/general/box-bottom.gif);'></div>
                                <?php } ?>
                        </div>
            ...
            <div id="ThemeboxesColumn">
                ...
                $creaturequery = $db
                  ->query(
                    'SELECT `boostname`, `looktype`, `lookfeet` , `looklegs` , `lookhead` , `lookbody` , `lookaddons` , `lookmount`   FROM `boosted_creature`'
                  )
                  ->fetch();
                ...
                $bossquery = $db
                  ->query(
                    'SELECT `boostname`, `looktypeEx`, `looktype`, `lookfeet` , `looklegs` , `lookhead` , `lookbody` , `lookaddons` , `lookmount`   FROM `boosted_boss`'
                  )
                  ->fetch();
                ...
                <div id="RightArtwork">
                    <img id="Creature"
                         src="<?= $config[
                           'outfit_images_url'
                         ] ?>?id=<?= $creaturetype ?>&addons=<?= $creatureaddons ?>&head=<?= $creaturehead ?>&body=<?= $creaturebody ?>&legs=<?= $creaturelegs ?>&feet=<?= $creaturefeet ?>&mount=<?= $creaturemount ?>"
                    ...
                    <?php else: ?>
                        <img id="Boss"
                             src="<?= $config[
                               'outfit_images_url'
                             ] ?>?id=<?= $bosstype ?>&addons=<?= $bossaddons ?>&head=<?= $bosshead ?>&body=<?= $bossbody ?>&legs=<?= $bosslegs ?>&feet=<?= $bossfeet ?>&mount=<?= $bossmount ?>"
                             alt="Boss of the Day"
                             title="Today's boosted boss: <?= ucwords(strtolower(trim($bossname))) ?>">
                    <?php endif; ?>
                ...
                <div id="Themeboxes">
                    ...
                    foreach ($config['boxes'] as $box) {
                      /** @var string $template_name */ $file = TEMPLATES . $template_name . '/boxes/' . $box . '.php';
                      if (file_exists($file)) {
                        include $file; ?>
                            <?php
                      }
